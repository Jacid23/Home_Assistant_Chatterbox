        ...
        try:
            for attempt in range(max_retries):
                try:
                    ...
                    # - Non-streaming: playback starts AFTER speak returns (at tts_end_time)
                    ...
                    while waited_time < max_wait_time:
                        ...
                        if not engine_active and (volume_changed or waited_time > 0.5):
                            # IMPORTANT: For non-streaming audio, playback starts NOW
                            # (when engine finishes), not when speak was called!
                            playback_start_time = asyncio.get_running_loop().time()
                            _LOGGER.info("Engine finished - playback starts NOW (non-streaming mode)")
