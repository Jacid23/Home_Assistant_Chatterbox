...
from .openaitts_engine import ChatterboxTTSEngine, StreamingAudioResponse
...
STORAGE_VERSION = 1
STORAGE_KEY = "chatterbox_tts_state"
...
async def async_setup_entry(
    ...
    """Set up Chatterbox TTS entities from a config entry."""
    _LOGGER.debug("Setting up Chatterbox TTS for config entry %s", config_entry.entry_id)
    ...
    if is_legacy:
        ...
        engine = ChatterboxTTSEngine(api_key, voice, model, speed, url)
        entity = ChatterboxTTSEntity(hass, config_entry, engine)
    ...
    if has_subentries:
        ...
        for subentry_id, subentry in config_entry.subentries.items():
            ...
            engine = ChatterboxTTSEngine(
                api_key, voice, model, speed, url,
                temperature=subentry.data.get(CONF_TEMPERATURE, DEFAULT_TEMPERATURE),
                exaggeration=subentry.data.get(CONF_EXAGGERATION, DEFAULT_EXAGGERATION),
                cfg_weight=subentry.data.get(CONF_CFG_WEIGHT, DEFAULT_CFG_WEIGHT),
                seed=subentry.data.get(CONF_SEED, DEFAULT_SEED),
            )
            entity = ChatterboxTTSEntity(hass, subentry, engine, config_entry)
...
class ChatterboxTTSEntity(TextToSpeechEntity, RestoreEntity):
    _attr_has_entity_name = True
    _attr_should_poll = False

    def __init__(self, hass: HomeAssistant, config: ConfigEntry, engine: ChatterboxTTSEngine, parent_entry: ConfigEntry = None) -> None:
        _LOGGER.debug("ChatterboxTTSEntity.__init__ called")
        self.hass = hass
        self._engine = engine
        self._config = config
        ...
        if is_subentry:
            ...
            self.entity_id = f"tts.chatterbox_tts_{safe_profile_name}"
            self._attr_name = f"Chatterbox TTS {profile_name}"
        else:
            ...
            if config.data.get(CONF_MODEL):
                ...
                self.entity_id = f"tts.chatterbox_tts_{model_suffix}"
                self._attr_name = f"Chatterbox TTS ({config.data.get(CONF_MODEL)})"
            else:
                # New-style main entry
                self.entity_id = "tts.chatterbox_tts"
        ...
        _LOGGER.info("Chatterbox TTS entity created: %s (engine speed: %s)", self.entity_id, self._engine._speed)
    ...
    def default_options(self) -> dict[str, Any]:
        ...
        return {
            ...
            CONF_TEMPERATURE: self._get_config_value(CONF_TEMPERATURE) or self._engine._temperature,
            CONF_EXAGGERATION: self._get_config_value(CONF_EXAGGERATION) or self._engine._exaggeration,
            CONF_CFG_WEIGHT: self._get_config_value(CONF_CFG_WEIGHT) or self._engine._cfg_weight,
            CONF_SEED: self._get_config_value(CONF_SEED, DEFAULT_SEED),
        }
    ...
    def device_info(self):
        ...
        if not device_unique_id:
            ...
            device_unique_id = self._config.data.get(CONF_URL, "chatterbox_tts")
        ...
        if is_subentry:
            ...
            device_info["name"] = "Chatterbox TTS"
    ...
    async def async_stream_tts_audio(
        ...
        try:
            ...
            # Chatterbox generation params
            temperature = options.get(CONF_TEMPERATURE) or self._get_config_value(CONF_TEMPERATURE) or self._engine._temperature
            exaggeration = options.get(CONF_EXAGGERATION) or self._get_config_value(CONF_EXAGGERATION) or self._engine._exaggeration
            cfg_weight = options.get(CONF_CFG_WEIGHT) or self._get_config_value(CONF_CFG_WEIGHT) or self._engine._cfg_weight
            seed = options.get(CONF_SEED)
            if seed is None:
                seed = self._get_config_value(CONF_SEED, DEFAULT_SEED)
                ...
                try:
                    if can_stream:
                        ...
                        audio_data = await self._get_processed_audio_for_streaming(
                            full_text, request.language, options, voice, model, speed,
                            temperature, exaggeration, cfg_weight, seed
                        )
    ...
    async def async_get_tts_audio(
        ...
        _LOGGER.info("TTS request - voice: %s, model: %s, speed: %s",
                     voice, model, speed)
        ...
        try:
            ...
            # Use the engine to get audio
            loop = asyncio.get_running_loop()
            
            # Pass all parameters to the engine
            audio_task = loop.run_in_executor(
                None,
                partial(
                    self._engine.get_tts,
