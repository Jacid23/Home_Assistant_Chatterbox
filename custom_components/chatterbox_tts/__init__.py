# __init__.py
"""Custom integration for Chatterbox TTS."""
...
SAY_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional("temperature"): vol.All(vol.Coerce(float), vol.Range(min=0.0, max=1.5)),
        vol.Optional("exaggeration"): vol.All(vol.Coerce(float), vol.Range(min=0.25, max=2.0)),
        vol.Optional("cfg_weight"): vol.All(vol.Coerce(float), vol.Range(min=0.2, max=1.0)),
...
async def async_migrate_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    if config_entry.version == 2 and config_entry.minor_version < 1:
        ...
        if config_entry.data.get(CONF_MODEL) or config_entry.data.get(CONF_VOICE):
            ...
            # This ensures tts.chatterbox_tts_tts_1 instead of tts.chatterbox_tts_default_tts_1
            profile_name = model
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up Chatterbox TTS and register the chatterbox_tts.say service."""
    ...
    async def update_listener(hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        _LOGGER.info("Config entry updated for Chatterbox TTS entry %s, reloading", entry.entry_id)
    ...
    if should_register_service:
        async def _handle_say(call: ServiceCall) -> None:
            ...
            options = {
                ...
                CONF_TEMPERATURE: data.get("temperature"),
                CONF_EXAGGERATION: data.get("exaggeration"),
                CONF_CFG_WEIGHT: data.get("cfg_weight"),
                CONF_SEED: data.get("seed"),
            }
        ...
        _LOGGER.info("Chatterbox TTS service 'say' registered successfully")
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if not is_subentry and hass.services.has_service(DOMAIN, SERVICE_NAME):
        ...
        if not other_entries:
            ...
            _LOGGER.info("Chatterbox TTS service 'say' unregistered")
