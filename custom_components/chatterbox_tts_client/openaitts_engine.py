"""
TTS Engine for Chatterbox TTS with optional streaming support.
"""
import json
import logging
import time
...
class ChatterboxTTSEngine:
    def __init__(self, api_key: str, voice: str, model: str, speed: float, url: str,
                 temperature: float = 0.8, exaggeration: float = 0.7,
                 cfg_weight: float = 0.5, seed: int = 0):
    ...
    def get_tts(
        ...
        Args:
            ...
            voice: Voice to use
            temperature: Randomness control (0.0-1.5)
            exaggeration: Expressiveness control (0.25-2.0)
            cfg_weight: CFG weight (0.2-1.0)
            seed: Generation seed (0 = random)
        ...
        headers = {
            ...
            "User-Agent": "HomeAssistant-Chatterbox-TTS"
        }
        if self._api_key:
        ...
        _LOGGER.debug("TTS API payload: model=%s, voice=%s, speed=%s, extra_keys=%s",
                     model, voice, speed,
                     [k for k in payload.keys() if k not in ("model", "input", "voice", "response_format", "speed")])
    ...
    async def async_get_tts_stream(
        ...
        """Stream TTS audio from Chatterbox API.
